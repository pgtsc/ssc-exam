    async function exportToExcel() {
        ...
        filteredData.forEach((row, index) => {
            worksheet.addRow({ sl: index + 1,  roll: row.Roll, trade: row.Trade,  status: row.Type, session:row.Session, institute: row.Institute,  room: row.Room || "" });
        });
    ...
    function calculateInstituteFee() {
        ...
        // Helper Functions....
        const isAllSubjects = (row) => {return subjectList.every(sub => row[sub] == 1);};
        ...
        // ===== Irregular =====
        let irregNoPracCount = 0;
        let irregSummary = {}; 
        let irregTotal = 0;

        data.forEach(r => {
            if (String(r.Type).toLowerCase() !== "irregular") return;

            let pracCount = getPracCount(r);
            if (pracCount === 0) {irregNoPracCount++; return;}
            if (isAllSubjects(r)) {pracCount = 6;}

            // summary count
            if (!irregSummary[pracCount]) irregSummary[pracCount] = 0;
            irregSummary[pracCount]++;

            // fee calc
            const total = FEES.CENTER + FEES.PRAC_CENTER + (pracCount * FEES.PRAC_PER_SUB);
            irregTotal += total;
        });
        ...
        // ===== Table rows =====
        let rows = "";
        Object.keys(irregSummary).sort((a,b)=>a-b).forEach(num => {
            const count = irregSummary[num];
        ...
        display.innerHTML = `
            <div class="p-5 bg-white shadow mx-auto" style="max-width:950px">
                ...
                <div class="mt-4 no-print">
                    <button class="btn btn-secondary" onclick="showUI('main-ui')">
                        <i class="fa-solid fa-arrow-left me-2"></i>Back
                    </button>
                    <button class="btn btn-success" onclick="printInstituteFee()">
                        <i class="fa-solid fa-print me-2"></i>Print
                    </button>
                    <button class="btn btn-primary" onclick="downloadInstituteFeeExcel()">
    ...
    async function downloadInstituteFeeExcel() {
        ...
        data.forEach(row => {
            const type = String(row.Type || "").trim().toLowerCase();
            let pracCount = getPracCount(row);
            let center = FEES.CENTER;
            let pracCenter = 0;
            let pracFee = 0;

            if (type === "regular") { pracCount = 6; pracCenter = FEES.PRAC_CENTER; pracFee = 6 * FEES.PRAC_PER_SUB;}
            else if (type === "irregular") {
                if (pracCount === 0) { pracCount = 0;}
                else {
                    if (isAllSubjects(row)) {pracCount = 6;}
                    pracCenter = FEES.PRAC_CENTER;
                    pracFee = pracCount * FEES.PRAC_PER_SUB;
                }
            }

            const total = center + pracCenter + pracFee;
            grandTotal += total;
            sheet.addRow([sl++, row.Roll, row.Type, center, pracCenter, pracCount, pracFee, total]);
        });
...
function printInstituteFee() {
    const content = document.getElementById('secondary-ui').innerHTML;
    ...
    printWindow.document.write(`
        <html>
        <head>
            <title>Institute Fee Report</title>
